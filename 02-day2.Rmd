...
Rmarkdown ermöglicht es uns, den Code zur Datenausewertung
...
ähnlich wie dieses Dokument es tut.^[In diesem Dokument zeige ich der Übersicht halber nicht von allen Chunks den kompletten Output, aber der Code lässt sich leicht mittels des Buttons oben rechts an den Blöcken in die Zwischenablage kopieren und selbst in R/RStudio ausprobieren.]
...
gegeben, damit die Begrüßungsnachricht des Tidyverses nicht in unserem Bericht landet.
...
Deutsche Excel Versionen nehmen statt Kommata (wie in comma-separated values)
jedoch Semikolons (da "," im Deutschen zur Trennung von Nachkommastellen
verwendet wird, im Englischen ist dafür "." zustängig). In diesem Fall
...
# Explaination of for-loop for simulation
for (i in 1:10) {
  print(i)
}
```
...
# median
median(results)
# Difference betweeen mean and median, their robustness to outliers!
...
# note that it calculates cummulative probabilities!
# default: P(X <= x)
...
Nullhypothese ("es gibt keinen Effekt vov Gender auf die
