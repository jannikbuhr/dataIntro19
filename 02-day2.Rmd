...
walk(1:10, print)
## Simulation
draw <- function(hat, size) {
  res <- sample(hat, size)
  sum(res == "l")
}

N <- 1000
long_hair_numbers <- map_int(1:N, ~draw(hat, 5))
...
Introduced in the 1920s by Ronald Fisher:
...
By convention: p ≤ 0.05 is “significant” p ≤ 0.01 is “highly significant”
...
1 - phyper(3, 4, 6, 4)
barplot(dhyper(0:5, 4, 6, 4), names.arg = 0:5)
# P(X ≥ 6) = 1 - P(X ≤ 5)
```
...
# Or:
# starwars <- read_csv("https://raw.githubusercontent.com/jannikbuhr/dataIntro19/master/data/01_starwars_example.csv")
head(starwars)
```
